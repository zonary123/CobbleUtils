...
public abstract class DataBaseUsers {
  ...
  public ServerPlayerEntity getPlayerOfflineOrOnline(UUID playerUUID) {
    ...
    if (requestedPlayer != null) {
      System.out.println("[DEBUG] Jugador online encontrado: " + requestedPlayer.getName().getString());
      return requestedPlayer;
    }

    System.out.println("[DEBUG] Jugador no está online, intentando cargar datos offline...");
    ...
    if (readViewOpt.isPresent()) {
      ...
      System.out.println("[DEBUG] Datos del jugador offline cargados correctamente.");
    } else {
      System.out.println("[DEBUG] No se pudieron cargar los datos del jugador offline.");
    }
